...
const chatWithAI = async (code, language, messages) => {
  const response = await API.post("/code/chat", { code, language, messages });
  return response.data.data;
};

export { translateCode, analyzeComplexity, optimizeCode, explainCode, chatWithAI };
